...
bool ModMenuPopup::init() {
    ...
    // --- Core tab: Speed Hack is the one real feature so far ---
    {
        ...
        // --- TPS Bypass: second real Core feature ---
        float tpsY = GRID_TOP_Y - ROW_HEIGHT;

        auto tpsToggle = CCMenuItemExt::createTogglerWithStandardSprites(
            0.6f,
            [](CCMenuItemToggler* toggle) {
                bool state = !toggle->isToggled();
                Mod::get()->setSavedValue("tpsbypass-enabled", state);
            }
        );
        tpsToggle->toggle(Mod::get()->getSavedValue<bool>("tpsbypass-enabled", false));
        tpsToggle->setPosition({COL_X[0], tpsY});
        menu->addChild(tpsToggle);

        auto tpsLabel = CCLabelBMFont::create("TPS Bypass", "bigFont.fnt");
        tpsLabel->setScale(0.35f);
        tpsLabel->setAnchorPoint({0.f, 0.5f});
        tpsLabel->setPosition({COL_X[0] + 20.f, tpsY});
        page->addChild(tpsLabel);

        auto tpsInput = TextInput::create(70.f, "240", "bigFont.fnt");
        tpsInput->setFilter("0123456789.");
        tpsInput->setMaxCharCount(6);
        tpsInput->setString(Mod::get()->getSavedValue<std::string>("tpsbypass-value", "240"));
        tpsInput->setPosition({COL_X[1] + 15.f, tpsY});
        tpsInput->setCallback([](std::string const& text) {
            Mod::get()->setSavedValue("tpsbypass-value", text);
        });
        page->addChild(tpsInput);
    }
